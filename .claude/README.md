...
```powershell
node .claude\scripts\gemini.mjs --dirs promptmeter "what does the greedy scheduler in planner.py actually optimize for?"
```
...
```
[context]   6 files · 49KB from promptmeter         what was collected
...
node .claude\scripts\gemini.mjs --dirs promptmeter --dry-context      # collector only
